...
```text
NanoLLM.exe /encode mergefile=tokenizer input=text.txt output=encoded ids=encoded
```

This writes:

- `encoded.tok`
- `encoded.ids`, if `ids=encoded` is supplied
...
The optional `.ids` file contains numeric vocabulary indexes. This is the
recommended input format for repeated training runs because `/train` can load it
directly without running BPE encoding again.
...
For faster repeated runs, train from a previously generated token-id file:

```text
NanoLLM.exe /train mergefile=tokenizer tokens=corpus output=model dim=64 window=4 epochs=3 neg=5 lr=0.025 seed=1
```
...
4. Encode the training corpus once as token ids:

```text
NanoLLM.exe /encode mergefile=tokenizer input=corpus.txt output=corpus ids=corpus
```

This writes `corpus.tok` for inspection and `corpus.ids`, a whitespace-separated
sequence of numeric token ids.

5. Train token embeddings from the reusable token-id file:

```text
NanoLLM.exe /train mergefile=tokenizer tokens=corpus output=model
```

6. Inspect the learned embedding file:
...
For benchmarking, prefer the pre-tokenized path:

```text
raw corpus text
-> /encode mergefile=tokenizer input=corpus.txt output=corpus ids=corpus
-> corpus.ids
-> /train mergefile=tokenizer tokens=corpus output=model
-> skip-gram training pairs
-> SGD updates
-> exported embedding matrix
```

This separates tokenizer cost from embedding-training cost. That matters when
you want to compare compiler options, dimensions, epochs, or learning rates
without paying the BPE encoding cost on every run.

### Token-id files

The `.ids` file is a plain text list of integer token ids:

```text
12 48 9 9 103 7
```

Each number is a row index into the `.voc` file and, later, into the embedding
matrix. The format is deliberately simple so it can be inspected, diffed, or
generated by small tools.
...
- encoding behavior
- vocabulary / merge / token / token-id file round-trips
- embedding trainer smoke tests
- training from pre-tokenized ids
